/** Uppercase section labels — use hero champagne on navy, champagne-dark on ivory/white */
export const eyebrowOnLight =
  "text-xs font-semibold uppercase tracking-[0.24em] text-brand-champagne-dark";
export const eyebrowOnLightWide =
  "text-xs font-semibold uppercase tracking-[0.28em] text-brand-champagne-dark";
export const eyebrowOnDark =
  "text-xs font-semibold uppercase tracking-[0.24em] text-brand-hero-champagne";
export const eyebrowOnDarkWide =
  "text-xs font-semibold uppercase tracking-[0.28em] text-brand-hero-champagne";

/** Building watchlist cards and other navy surfaces */
export const cardAccent = "text-brand-hero-champagne";
export const cardAccentMuted = "text-brand-hero-champagne/75";
export const cardAccentLabel =
  "text-[0.56rem] font-semibold uppercase tracking-[0.2em] text-brand-hero-champagne/75";
export const cardAreaLabel =
  "text-[0.68rem] font-medium tracking-[0.14em] text-brand-hero-champagne sm:text-xs";
export const cardReportLink =
  "text-[0.62rem] font-semibold uppercase tracking-[0.2em] text-brand-hero-champagne transition hover:text-brand-ivory";

export const cardAmenityLabel =
  "text-[0.52rem] font-medium leading-snug tracking-[0.05em] text-brand-ivory/68";
