...
- **AI Chat Assistant with CRM Lead Capture**: GPT-4o-mini powered assistant with an ULTRA-AGGRESSIVE "Jordan Belfort" personality - relentless, persistent, never takes no for an answer. Uses emojis heavily (🏆💰🎯🔥💎), sends multiple follow-ups even without response, and NEVER backs off until getting complete info (Name, Timeline, Budget, Email, Phone). Features rapid-fire button-style multiple choice options with emoji indicators (🅰️🅱️🅲️🅳️), keeps conversations alive with 5-10+ messages, ignores rejection and rephrases questions. Includes automatic, hidden `LEAD_DATA` JSON block extraction, session-based tracking, database persistence to the `leads` table with calculated lead scores, and voice input support. Dark navy header with white text, medium blue user bubbles, and gold agent bubbles with black text. All interactive buttons feature addictive crunchy chime sound effects using Web Audio API for satisfying user engagement.
