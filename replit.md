...
### Data Storage
Drizzle ORM with the neon-http adapter is used for PostgreSQL connections. The database schema includes `users` for authentication, `leads` for CRM capturing detailed qualification fields, and `contentItems` for the internal Content Studio (video production workflow management with stages: ideation, legal, ready, completed). Database migrations are managed with `npm run db:push`.
...
- **Process Flow Section**: Visual representation of the platform's unique approach showing the 4-step journey: Client → Encrypted Trust Layer (Anonymous LLC structure for privacy) → Bidding Profile (leverage display with cash and buying power) → Brokers Compete (reverse auction model). Emphasizes how the platform flips traditional real estate power dynamics. Implemented as horizontal swipeable Embla carousel with compact card layout.
- **Audiobooks Page**: Premium audiobook library featuring 9 audiobooks (8 regular + 1 featured) with standardized 2-hour duration and $10 pricing. **Featured Audiobook**: "Awaken the Straight Guy Within" by Agent Kammer displayed in prominent 2x-width card with 3D book cover mockup, author professional headshot photo, cool gradient background, and enhanced visual hierarchy. **Personal Transformation** section (4 books): "The Wealth Magnet," "Sacred Partnership," "Quantum Manifestation," and "The Power Within." **Luxury Real Estate Mastery** section (4 books): "The Art of Luxury Living," "Wealth Architecture," "Negotiation Mastery," and "Global Property Intelligence." Design features glossy black cards with custom icon artwork (classy black with gold details, hand-painted oil illustration style, modernized sticker-like emblems without borders), frosted glass covers, gold accent lines, radial gold glows, premium gold gradient buttons, and white/gold typography. Each card displays custom icon image, author, 2h duration, $10 price, 4.7-5.0 star rating, description, and "Listen Now" button. All audiobook icons are AI-generated custom artwork replacing generic Lucide icons. Accessible via footer link with headphones icon. Includes WhatsApp number capture for audiobook samples with lead tracking.
- **Ecourses Page**: Premium online courses for luxury real estate professionals featuring 3 courses with gold/navy branding. **Featured Course**: "Perfect Decision-Making: The Strategic Neutrality Method" (1 hour, $100) - teaches neutral decision-making framework inspired by Swiss neutrality, externalizing thoughts, systematic pros/cons analysis, and becoming the impartial judge of your own mind. Includes 6 learning highlights covering neutrality framework, bias externalization, counterargument analysis, and real estate applications. **Additional Courses**: "Luxury Client Psychology" (1.5 hours, $150, 4.9 rating) - decoding UHNW client mindset and behavioral patterns; "Negotiation Mastery for Elite Agents" (2 hours, $200, 5.0 rating) - advanced tactics for $10M+ transactions and competitive bidding. Featured course displayed in prominent 2-column layout with Scale icon, rating (5.0), student count (847). Regular courses in 2-column grid with custom icons (Brain, Handshake). All courses feature detailed highlights, instructor info, pricing, duration, student counts, and gold gradient "Enroll Now" buttons. Accessible via footer link with GraduationCap icon. Enrollment flow pending future implementation.
- **Content Studio (Internal)**: Private management system for social media video strategy accessible only via direct URL (/studio). Features workflow stages: Ideation → Legal Review → Ready to Shoot → Completed. Softer grey background (slate-600/700) for visual comfort with scrollable tabs on mobile. Includes full CRUD operations for managing video content items with fields for title, description, script content, category, tags, notes, legal status, file URLs, and publishing destinations. **Publishing Destinations**: Track where content will be published (YouTube, Instagram Reels, LinkedIn) with checkbox selection in forms and colored badge display on cards - ready for future social media API integration. All API routes protected with Zod schema validation and stage enumeration enforcement. Database persistence via contentItems table with publishingDestinations array field and timestamps.
