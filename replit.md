...
- **AI Chat Assistant with CRM Lead Capture**: GPT-4o-mini powered assistant with a bold, charismatic "Jordan Belfort" personality - confident, fun, and classy. Features aggressive lead qualification strategy that quickly captures name, timeline, budget/financing, and contact info within first 3-4 messages using button-style options. Includes automatic, hidden `LEAD_DATA` JSON block extraction, session-based tracking, database persistence to the `leads` table with calculated lead scores, and voice input support. Dark navy header with white text, medium blue user bubbles, and gold agent bubbles with black text. All interactive buttons feature addictive crunchy chime sound effects using Web Audio API for satisfying user engagement.
