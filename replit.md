...
- **Content Studio (Internal)**: Private management system for social media video strategy accessible only via direct URL (/studio). Features workflow stages: Ideation → Legal Review → Ready to Shoot → Completed. Futuristic dark UI with glassmorphic tabs, drag-and-drop stage progression, and gold accent buttons. Includes full CRUD operations for managing video content items with fields for title, description, script content, category, tags, notes, legal status, and file URLs. All API routes protected with Zod schema validation and stage enumeration enforcement. Database persistence via contentItems table with timestamps.
